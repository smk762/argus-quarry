...
pool and answers `503` with `{"status": "unavailable", "database": "..."}` when
it cannot be served (missing, unmigrated, or an unreadable WAL), so point an
orchestrator's *readiness* probe at `/ready` to shed a container that is up but
has no data to serve, and its *liveness* probe at `/health`.

| Endpoint | Returns |
|---|---|
| `GET /health` | Liveness: `200 {status, service, version, quarry_home}` while the process is up |
| `GET /ready` | Readiness: `200 {status: ready, database: ok}`, or `503 {status: unavailable, database}` when the pool cannot be served |
